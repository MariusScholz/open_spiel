...
flags.DEFINE_integer("player1_pop_id", 0, "Population member ID for player 1")
...
#   print("Starting round-robin evaluation of all bot pairs...\n")
#   results_matrix = np.zeros((pop_size, pop_size))

#   for i in range(pop_size):
#     for j in range(pop_size):
#       if i == j:
#         continue  # Skip self-play if undesired

#       print(f"\nEvaluating: {roshambo_bot_names[i]} (Player 0) vs {roshambo_bot_names[j]} (Player 1)")
#       agents = [
#           create_roshambo_bot_agent(0, num_actions, roshambo_bot_names, i),
#           create_roshambo_bot_agent(1, num_actions, roshambo_bot_names, j)
#       ]
#       avg_eval_returns = eval_agents(env, agents, 2, 100)
#       results_matrix[i][j] = avg_eval_returns[0]  # Store Player 0's average return

#   print("\nFinal Results Matrix (rows = Player 0, columns = Player 1):")
